...
def main():
    parser = argparse.ArgumentParser(
        description="G1 21-DOF PKL -> AMP CSV (lin_vel, ang_vel, joint_pos, joint_vel, key_body_pos)"
    )
    ...
    parser.add_argument("--config", type=str, default="legged_lab/tools/config/g1_21dof.yaml", help="YAML config for GMR→Lab joint mapping")
    ...
    for pkl_file in pkl_files:
        ...
        # --- key_body_pos (FK via Isaac Sim, with Lab-order joints) ---
        # Remap dof_pos to Lab order just for FK; CSV still stores GMR order.
        ...
        for t in range(num_frames):
            dof_pos_lab = dof_pos_data[t, gmr_to_lab]
