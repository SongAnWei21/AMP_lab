...
import legged_lab.mdp.rewards_g1 as rwd
...
@configclass
class G1_WalkRewardCfg:
    # -- task (engineai_amp exact: track_lin_vel_xy_yaw_frame_exp + track_ang_vel_z_world_exp)
    track_lin_vel_xy_exp = RewTerm(
        func=rwd.track_lin_vel_xy_yaw_frame_exp,
        weight=2.0,
        params={"std": math.sqrt(0.2)},  # sigma=5 → std=sqrt(1/5)
    )
    track_ang_vel_z_exp = RewTerm(
        func=rwd.track_ang_vel_z_world_exp,
        weight=2.5,
        params={"std": math.sqrt(0.2)},
    )

    # -- base (engineai_amp exact)
    base_orientation = RewTerm(func=rwd.base_orientation_ea, weight=1.0)
    base_height = RewTerm(
        func=rwd.base_height_tracking_ea,
        weight=0.4,
        params={"target_height": 0.82},
    )

    # -- joint deviations (engineai_amp exact: joint_deviation_exp, G1 joint names)
    waist_pos = RewTerm(
        func=rwd.joint_deviation_exp,
        weight=0.3,
        params={
    ...
    # -- feet (engineai_amp exact)
    feet_contact = RewTerm(
        func=rwd.feet_contact_fixed,
        weight=0.25,
        params={
    ...
    feet_air_time_dense = RewTerm(
        func=rwd.feet_air_time_positive_biped,
        weight=1.25,
        params={
            "sensor_cfg": SceneEntityCfg("contact_sensor", body_names=[".*ankle_roll_link"]),
            "threshold": 0.5,
    ...
    # -- regularization (engineai_amp exact)
    dof_pos_limits = RewTerm(func=mdp.joint_pos_limits, weight=-10.0)
    energy_cost = RewTerm(
        func=rwd.energy_cost_with_curriculum,
        weight=-0.004,
        params={
            "start_scale": 0.1,
            "power": 0.8,
    ...
    dof_vel = RewTerm(func=rwd.joint_vel_l2, weight=-1.0e-5)
    dof_acc = RewTerm(func=mdp.joint_acc_l2, weight=-1.25e-8)
    action_rate = RewTerm(
        func=rwd.action_rate_with_curriculum,
        weight=-0.06,
        params={
            "start_scale": 0.1,
            "power": 0.8,
    ...
    action_smoothness = RewTerm(
        func=rwd.action_smoothness_with_curriculum,
        weight=-0.04,
        params={
            "start_scale": 0.1,
            "power": 0.8,
    ...
    dof_torque = RewTerm(func=mdp.joint_torques_l2, weight=-1.0e-6)
...
@configclass
class G1_WalkAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    ...
    clip_actions = None
    save_interval = 100
    runner_class_name = "G1_Walk_AmpOnPolicyRunner"
    ...
    # --- AMP参数 (engineai_amp格式: joint_pos*9 + lin_vel*7 = 24 dims/frame) ---
    amp_reward_coef = 1.0
    amp_motion_files = [
    ...
    min_normalized_std = [0.05] * 24  # must match frame_dim
    amp_frame_dim = 24  # joint_pos(21) + lin_vel(3)
