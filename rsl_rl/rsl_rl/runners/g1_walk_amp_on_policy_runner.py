...
from rsl_rl.utils.g1_walk_motion_loader import G1_Walk_AMPLoader


class G1_Walk_AmpOnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        # init amp loader (G1_Walk: reads CSV in Isaac Lab articulation order)
        amp_data = G1_Walk_AMPLoader(
            device=device,
            motion_files=train_cfg["amp_motion_files"],
            preload_transitions=True,
            num_preload_transitions=train_cfg["amp_num_preload_transitions"],
        )
        amp_frame_dim = train_cfg.get("amp_frame_dim", amp_data.observation_dim)
        amp_normalizer = Normalizer(amp_data.observation_dim)
        discriminator = Discriminator(
            amp_data.observation_dim * 2,  # current+next frame concatenated
            train_cfg["amp_reward_coef"],
            train_cfg["amp_discr_hidden_dims"],
            device,
            train_cfg["amp_task_reward_lerp"],
        ).to(self.device)
        min_std = torch.zeros(amp_frame_dim, device=self.device, requires_grad=False)
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        # create buffers for logging extrinsic and intrinsic rewards (RND)
        if self.alg.rnd:
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    ...
                    combined_rewards, d = self.alg.discriminator.predict_amp_reward(
                        amp_obs, next_amp_obs_with_term, rewards, normalizer=self.alg.amp_normalizer
                    )
                    style_rewards = self.alg.discriminator.amp_reward_coef * torch.clamp(
                        1 - (1 / 4) * torch.square(d.squeeze() - 1), min=0
                    )
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            log_string += f"""{'Mean AMP style reward:':>{pad}} {statistics.mean(locs['style_rewbuffer']):.2f}\n"""
