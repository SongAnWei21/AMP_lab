...
class AMPPPO:
    ...
    def update(self):  # noqa: C901
        ...
        # ==========================================
        # PPO 核心数据生成器 (用于更新 Actor 和 Critic 网络)
        # ==========================================
        if self.policy.is_recurrent:
            # 如果策略网络是循环神经网络 (RNN/LSTM/GRU)，数据不能随意打乱，必须保持时间序列的连续性。
            # 所以调用 recurrent_mini_batch_generator 按照序列切分 mini-batch。
            generator = self.storage.recurrent_mini_batch_generator(self.num_mini_batches, self.num_learning_epochs)
        else:
            # 如果策略网络是普通的全连接网络 (MLP)，数据可以完全随机打乱。
            # 所以调用标准的 mini_batch_generator。
            generator = self.storage.mini_batch_generator(self.num_mini_batches, self.num_learning_epochs)

        # ==========================================
        # AMP 判别器数据生成器 (用于对抗模仿学习)
        # ==========================================
        # 计算每个 mini-batch 的样本数量 (对应之前提到的 original_batch_size)
        # 公式: 总样本数(环境数 * 每个环境步数) // 切分的批次数量
        amp_batch_size = self.storage.num_envs * self.storage.num_transitions_per_env // self.num_mini_batches

        # 策略生成数据 (Policy Data)
        # 从 amp_storage 中提取当前机器人在仿真环境里“自己踩出来”的观测数据。
        # 判别器的任务是认出这些数据是“假的”（机器人生疏的动作）。
        amp_policy_generator = self.amp_storage.feed_forward_generator(
            self.num_learning_epochs * self.num_mini_batches, # 总共需要生成多少个批次
            amp_batch_size,                                   # 每个批次的大小
        )
        
        # 专家示范数据 (Expert Data)
        # 从 amp_data 中提取真实的动捕数据
        # 判别器的任务是认出这些数据是“真的”
        # 注意：这里专家数据的 batch_size 和上面策略数据的 batch_size 保持完全一致，
        # 是为了保证判别器在训练时，看到的“真数据”和“假数据”比例是 1:1，防止样本不平衡。
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            # Symmetry loss  对称性loss  用于强制策略学习左右对称的步态
            if self.symmetry:
                # 获取对称动作
                # 如果之前在 PPO 数据准备阶段没有做过对称数据增强，这里就需要对观测数据进行镜像处理
                if not self.symmetry["use_data_augmentation"]:
                    data_augmentation_func = self.symmetry["data_augmentation_func"]
                    # 将原始的 obs_batch 输入镜像函数，进行增强。
                    # 执行后，obs_batch 的大小会翻倍：前半部分是原观测数据，后半部分是对称翻转后的镜像观测数据
                    obs_batch, _ = data_augmentation_func(
                        obs=obs_batch, actions=None, env=self.symmetry["_env"], obs_type="policy"
                    )
                    # 计算样本的增强倍数（通常 num_aug 为 2，因为包含了原数据 + 1份镜像数据）
                    num_aug = int(obs_batch.shape[0] / original_batch_size)

                # 策略网络对整个 obs_batch（包含原数据和镜像数据）进行前向推理。
                # 得到的 mean_actions_batch 前半部分是策略对“原状态”的动作均值，后半部分是策略对“镜像状态”的动作均值
                mean_actions_batch = self.policy.act_inference(obs_batch.detach().clone())

                # 计算理论上完美的对称动作 (symmetrically augmented actions)
                # 注意：我们假设 batch 的前半部分（第一份 augmentation）是原始数据。
                # 我们这里不使用之前环境交互时采样得到的 action_batch，因为那些动作是从概率分布中采样出来的（带有随机性/探索噪声）。
                # 为了训练稳定性，对称性损失是直接对比动作分布的均值 (mean of the distribution)。
                
                # 提取出策略对“原始观测”的确定性动作预测，前半部分原始动作
                action_mean_orig = mean_actions_batch[:original_batch_size]
                
                # 将原始动作通过镜像函数翻转，算出“理论上”如果遇到对称状态，机器人应该做出的完美动作
                # actions_mean_symm_batch 的后半部分就是这个“标准答案”
                _, actions_mean_symm_batch = data_augmentation_func(
                    obs=None, actions=action_mean_orig, env=self.symmetry["_env"], obs_type="policy"
                )

                # 计算均方误差损失（MSELoss）。
                # 我们跳过前半部分（原始数据比对没有意义），只对比后半部分（镜像数据部分）。
                mse_loss = torch.nn.MSELoss()
                symmetry_loss = mse_loss(
                    # 策略网络看到“镜像状态”后，实际输出的动作 ，后半部分
                    mean_actions_batch[original_batch_size:], 
                    # 理论上应该输出的“完美镜像动作” (作为目标标签，不需要计算梯度，所以加上 .detach())
                ...
                if self.symmetry["use_mirror_loss"]:
                    ...
                    # 如果配置中设为 False，则说明对称性只用于统计/日志记录，截断梯度不参与反向传播
